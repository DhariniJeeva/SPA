...
class App extends Component {
  render() {
     //filter out the duplicates from json data
    let banks = bankData.products;
    banks = banks.filter((value, index) =>
    index === banks.findIndex((dup) => (dup.name === value.name && dup.type === value.type)))
    return (
      <div className="App">
       <h2>Financial Institution Search</h2>
       <Search data = {banks}/>
      </div>
    );
  }
